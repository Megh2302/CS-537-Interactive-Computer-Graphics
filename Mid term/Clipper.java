...
public class Clipper {
    ...
    //
    ///
    public Matrix[] clipPolygon(int in, Matrix[] vertices, float llx, float lly, float urx, float ury)
    {
        ...
        for(int i = 0; i < in; i++)
        {
            inpoints.get(0).add((float)vertices[i].get(0,0));
            inpoints.get(1).add((float)vertices[i].get(1,0));
        }
        int ol;
        ol= in;
        ArrayList<ArrayList<Float>> outpoints = new ArrayList<>();
        
        //Performing algo on each boundry
        ...
        Matrix[] op = new Matrix[ol];
        for(int i = 0; i < ol; i++)
        {
            double[][] val = {{outpoints.get(0).get(i)}, {outpoints.get(1).get(i)}, {1}};
            op[i] = new Matrix(val);
    ...
    //Checking the point whether it is inside the boundary
   ...
   // Adding point to the output vertices
    ...
    //the point where the line intersecting the boundary
